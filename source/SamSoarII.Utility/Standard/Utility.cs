...
internal static class Utility
{
	...
	public static bool AreStreamsEqual(Stream left, Stream right)
	{
		...
		try
		{
			while (num2 < num)
			{
				Standard.Assert.AreEqual(num2, num3);
			...
			Standard.Assert.AreEqual(num4, num5);
			Standard.Assert.AreEqual(num2, num3);
			Standard.Assert.AreEqual(num, num2);
			return true;
		}
		finally
		{
	...
	public static IntPtr GenerateHICON(ImageSource image, Size dimensions)
	{
		...
		{
			item = GetBestMatch(bitmapFrame.Decoder.Frames, (int)dimensions.Width, (int)dimensions.Height);
		}
		else
		{
	...
	public static void SafeRelease<T>(ref T comObject) where T : class
	{
		...
		if (val != null)
		{
			Standard.Assert.IsTrue(Marshal.IsComObject(val));
	...
	{
		Standard.Assert.IsNotNull(source);
		Standard.Assert.IsFalse(string.IsNullOrEmpty(propertyName));
		if (source.Length != 0)
		{
			source.Append(' ');
		}
	...
	public static string GenerateToString<T>(T @object) where T : struct
	{
		...
		foreach (PropertyInfo propertyInfo in properties)
		{
			...
			Standard.Assert.AreEqual(0, propertyInfo.GetIndexParameters().Length);
	...
	{
		Standard.Assert.IsNotNull(source);
		Standard.Assert.IsNotNull(destination);
	...
	{
		Standard.Assert.IsNotNull(left);
		Standard.Assert.IsNotNull(right);
		Standard.Assert.IsTrue(cb <= Math.Min(left.Length, right.Length));
	...
	public static string UrlEncode(string url)
	{
		...
		if (flag)
		{
			...
			foreach (byte b2 in array4)
			{
				...
				array3[actual++] = 37;
				array3[actual++] = _IntToHex((b2 >> 4) & 0xF);
				array3[actual++] = _IntToHex(b2 & 0xF);
			}
			array = array3;
			Standard.Assert.AreEqual(array3.Length, actual);
		}
	...
	{
		Standard.Assert.BoundedInteger(0, n, 16);
		if (n <= 9)
		{
			return (byte)(n + 48);
		}
	...
	private static int _HexToInt(char h)
	{
		...
		Standard.Assert.Fail("Invalid hex character " + h);
		return -1;
	}
	...
	public static void AddDependencyPropertyChangeListener(object component, DependencyProperty property, EventHandler listener)
	{
		if (component != null)
		{
			Standard.Assert.IsNotNull(property);
			Standard.Assert.IsNotNull(listener);
