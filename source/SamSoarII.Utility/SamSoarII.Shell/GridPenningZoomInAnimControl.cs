...
public class GridPenningZoomInAnimControl : UserControl
{
	...
	protected static readonly DependencyProperty TickProperty = DependencyProperty.Register("Tick", typeof(int), typeof(GridPenningZoomInAnimControl), new PropertyMetadata(0, OnPropertyChanged_Tick));

	protected static readonly DependencyProperty RectProperty = DependencyProperty.Register("Rect", typeof(Rect), typeof(GridPenningZoomInAnimControl), new PropertyMetadata(default(Rect), OnPropertyChanged_Rect));
	...
	{
		if (d is GridPenningZoomInAnimControl)
		{
			((GridPenningZoomInAnimControl)d).OnTickChanged(e);
		}
	}
	...
	protected override void OnRender(DrawingContext ctx)
	{
		...
		double num = rect.Top + (rect2.Top - rect.Top) * (double)Tick / (double)TickMax;
		double num2 = rect.Bottom + (rect2.Bottom - rect.Bottom) * (double)Tick / (double)TickMax;
		double num3 = rect.Left + (rect2.Left - rect.Left) * (double)Tick / (double)TickMax;
		double num4 = rect.Right + (rect2.Right - rect.Right) * (double)Tick / (double)TickMax;
		...
		ctx.DrawRectangle(Brush_Lookbar, null, new Rect(point.X - Thickness_Lookbar / 2.0, point.Y - Length_Look / 2.0, Thickness_Lookbar, Length_Look));
